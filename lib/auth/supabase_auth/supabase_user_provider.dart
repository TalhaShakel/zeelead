...
class TemplateZleadsSupabaseUser extends BaseAuthUser {
  TemplateZleadsSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> templateZleadsSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = TemplateZleadsSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
